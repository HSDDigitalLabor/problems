...
def test_string_guess():
    """zahlenjagd.py lehnt nicht-numerischen Schuss ab"""
    check50.run(f"python3 {FILE_NAME}").stdin("dog", prompt=True).reject()
...
def test_out_of_range_large():
    ...
    output = "Zu hoch!"
    check50.run("python3 testing.py").stdin("8", prompt=True).stdout(
...
def test_nonpositive_shot():
    ...
    check50.run(f"python3 {FILE_NAME}").stdin("0", prompt=True).stdout(
        reject_regex("1 und 100"), "Bitte gib eine Zahl zwischen 1 und 100 ein."
    ).kill()
    check50.run(f"python3 {FILE_NAME}").stdin("-50", prompt=True).stdout(
        reject_regex("1 und 100"), "Bitte gib eine Zahl zwischen 1 und 100 ein."
    ).kill()
...
def test_too_large():
    ...
    output = "Zu hoch!"
    check50.run("python3 testing.py").stdin("18", prompt=True).stdout(
        regex(output), output, regex=True
    ).reject()
...
def test_just_right():
    ...
    output = "Volltreffer!"
    check50.run("python3 testing.py").stdin("4", prompt=True).stdout(
        regex(output), output, regex=True
    ).exit()
...
def test_too_small():
    ...
    output = "Zu niedrig!"
    check50.run("python3 testing.py").stdin("2", prompt=True).stdout(
        regex(output), output, regex=True
    ).reject()
